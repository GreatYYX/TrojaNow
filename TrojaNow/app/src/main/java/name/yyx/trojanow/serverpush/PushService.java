...
{
	// this is the log tag
	public static final String		TAG = "TrojaNow PushService";

	// the IP address, where your MQTT broker is running.
	private static final String		MQTT_HOST = "trojanow.yyx.name";
	...
	public static String			MQTT_CLIENT_ID = "TrojaNow";
...
//	public static final String		PREF_DEVICE_ID = "deviceID";
    public static final String		PREF_MQTT_USER = "MQTT user";
	// We store the last retry interval
	public static final String		PREF_RETRY = "retryInterval";

	// Notification title
	public static String			NOTIF_TITLE = "TrojaNow";
	// Notification id
	...
	private synchronized void connect() {		
		...
		String mqttUser = mPrefs.getString(PREF_MQTT_USER, null);
		// Create a new connection only if the mqtt user is not NULL
		if (mqttUser == null) {
			log("Mqtt user not found.");
		} else {
			try {
				mConnection = new MQTTConnection(MQTT_HOST, mqttUser);
			} catch (MqttException e) {
				// Schedule a reconnect, if we failed to connect
				log("MqttException1: " + (e.getMessage() != null ? e.getMessage() : "NULL"));
	        	if (isNetworkAvailable()) {
	...
	private synchronized void keepAlive() {
		try {
			...
			log("MqttException2: " + (e.getMessage() != null? e.getMessage(): "NULL"), e);
	...
	private class MQTTConnection implements MqttSimpleCallback {
		...
		public MQTTConnection(String brokerHostName, String initTopic) throws MqttException {
	    	...
	    	String mqttConnSpec = "tcp://" + brokerHostName + "@" + MQTT_BROKER_PORT_NUM;
	        	...
	        	String clientID = MQTT_CLIENT_ID + "/" + mPrefs.getString(PREF_MQTT_USER, "");
	        	mqttClient.connect(clientID, MQTT_CLEAN_START, MQTT_KEEP_ALIVE);

		        // register this client app has being able to receive messages
				mqttClient.registerSimpleHandler(this);
				
				// Subscribe to an initial topic, which is combination of client ID and mqtt user.
		...
		public void disconnect() {
			try {			
				...
				log("MqttException3" + (e.getMessage() != null? e.getMessage():" NULL"), e);
			}
		}
		/*
		...
		public void sendKeepAlive() throws MqttException {
			...
			publishToTopic(MQTT_CLIENT_ID + "/keepalive", mPrefs.getString(PREF_MQTT_USER, ""));
		}		
	}
}